    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto px-4 py-8">
      ...
      <form action={handleSubmit} className="relative w-full max-w-sm space-y-4 rounded-2xl border border-white/10 bg-base-900 p-6">
        ...
        <div>
          ...
          <input name="display_order" type="number" defaultValue={cert?.display_order ?? 0} className="w-full rounded-lg border border-white/10 bg-base-800 px-3 py-2 text-white" />
        </div>
  ...
  return (
    <div>
      <div className="flex justify-end">
        <button type="button" onClick={() => setModal({ open: true })} className="flex items-center gap-2 rounded-lg bg-accent-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-accent-600">
        ...
        {certifications.length === 0 ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {certifications.map((certification) => (
              <div key={certification.id} className="flex items-center justify-between rounded-xl border border-white/5 bg-base-800 p-4">
                <div>
                  <p className="font-medium text-white">{certification.title}</p>
                  <p className="text-sm text-slate-400">{certification.organization}{!certification.visible && ' - hidden'}</p>
                </div>
                <div className="flex gap-3">
                  <button type="button" onClick={() => setModal({ open: true, cert: certification })} className="text-slate-400 hover:text-white"><Pencil size={16} /></button>
                  <ConfirmDialog
                    trigger={<button type="button" className="text-slate-400 hover:text-red-400"><Trash2 size={16} /></button>}
                    title="Delete this certification?"
                    description="This cannot be undone."
                    onConfirm={async () => { const result = await deleteCertification(certification.id); if (result.error) { toast.error(result.error); return; } toast.success('Certification deleted'); refresh(); }}
                  />
                </div>
              </div>
            ))}
